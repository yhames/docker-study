...
협업을 하거나 프로젝트를 서버에서 배포하기 위해서는 도커 이미지 혹은 컨테이너를 공유해야합니다.

Dockerfile과 빌드하기 위한 소스코드를 공유하여 각자 빌드하여 사용할 수도 있지만,

Dockerhub 등을 사용하여 빌드된 이미지 자체를 공유하면 빌드하는 시간을 줄일 수 있기 때문에 효율적입니다. 

도커 이미지를 공유하는 방법은 Dockerhub와 같은 서비스를 이용하거나
Private Registry를 구축하여 사용할 수 있습니다.

Private Registry를 사용하면 이미지를 전체 공개하지 않고 팀 내에서만 사용할 수 있습니다.

도커 이미지를 Dockerhub 업로드하기 위해 먼저 Dockerhub에 새로운 레포지토리를 생성합니다.

다음으로 레포지토리 이름과 태그를 지정하여 이미지를 빌드합니다.

```bash
docker build -t <repository>/<image>:<tag> .
```

또는 빌드된 이미지에 이름과 태그를 변경하여 사용할 수도 있습니다.

```bash
docker tag <image_id> <repository>/<image>:<tag>
```

이후에 push 명령어를 사용하면 이미지를 Dockerhub에 업로드할 수 있습니다.

```bash
docker push <repository>/<image>:<tag>
```

만약 로그인이 되어있지 않아서 이미지를 업로드할 수 없다면 `login` 명령어를 사용하여 로그인합니다.

```bash
docker login
```

도커 이미지를 가져오기 위해서는 위에서 지정한 레포지토리 이름과 태그를 사용하여 `pull` 명령어를 사용합니다.

```bash
docker pull <repository>/<image>:<tag>
```

`pull` 명령어를 사용하여 이미지를 가져오면 해당 이미지를 사용하여 컨테이너를 생성할 수 있습니다.

```bash
docker run <repository>/<image>:<tag>
```

> 만약 로컬에 이미 이미지가 존재한다면, docker run 명령어를 사용해도 최신 버전의 이미지를 가져오지 않습니다.  
> 따라서 이미지를 업데이트하려면 `pull` 명령어를 사용하여 이미지를 가져온 후 컨테이너를 생성해야 합니다.

## 정리

도커는 이미지와 컨테이너를 관리하는 도구입니다.

이미지는 컨테이너를 생성하기 위한 템플릿이며, 컨테이너는 이미지를 통해 생성된 런타임 인스턴스입니다.

이미지는 명령어 레이어를 기반으로 구성되어 있으며, 각 레이어는 캐싱되어 이미지 재구축을 효율적으로 수행합니다.

컨테이너는 명령어 레이어 최상단에서 추가적인 레이어를 생성하여 실행되며,
이미지의 코드와 환경을 직접 복사하지 않고 이를 단지 활용하여 실행됩니다

또한 도커 이미지는 Dockerhub와 같은 서비스를 사용하여 공유할 수 있습니다.
